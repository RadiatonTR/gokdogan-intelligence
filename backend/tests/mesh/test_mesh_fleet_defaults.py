...
def test_fleet_defaults_apply_when_join_enabled(monkeypatch):
    ...
    try:
        ...
        # Discovery metadata may be bundled; authentication credentials may not.
        assert effective_peer_push_secret() == ""
    finally:
        get_settings.cache_clear()


def test_fleet_peer_secret_requires_operator_configuration(monkeypatch):
    from services.config import get_settings

    monkeypatch.setenv("MESH_INFONET_FLEET_JOIN", "true")
    monkeypatch.setenv("MESH_PEER_PUSH_SECRET", "operator-owned-secret-at-least-32-chars")
    get_settings.cache_clear()
    try:
        assert effective_peer_push_secret() == "operator-owned-secret-at-least-32-chars"
    finally:
        get_settings.cache_clear()
