...
# Authentication secrets are deliberately NOT bundled. A shared value committed
# in source is public by definition and cannot authenticate a peer. Operators who
# explicitly join a fleet must provision MESH_PEER_PUSH_SECRET (or per-peer
# MESH_PEER_SECRETS) out-of-band.
...
def effective_peer_push_secret() -> str:
    try:
        ...
        pass
    # Never fall back to a source-controlled fleet secret. If no operator-owned
    # credential is configured, authenticated peer push/sync remains unavailable.
    return ""
