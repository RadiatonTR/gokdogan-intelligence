...
DATA_DIR = Path(os.environ.get("SB_DATA_DIR", str(Path(__file__).resolve().parents[2] / "data")))
...
def _write_vote_storage_state(state: dict[str, Any]) -> None:
    ...
    write_domain_json(LEDGER_DOMAIN, VOTE_SALT_STATE_FILE, payload, base_dir=DATA_DIR)
...
class ReputationLedger:
    ...
    def _flush(self):
        ...
        try:
            ...
            write_domain_json(LEDGER_DOMAIN, LEDGER_FILE.name, data, base_dir=DATA_DIR)
            LEDGER_FILE.unlink(missing_ok=True)
            self._dirty = False
        except Exception as e:
