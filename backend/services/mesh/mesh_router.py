...
class MeshtasticTransport:
    ...
    def send(self, envelope: MeshEnvelope, credentials: dict) -> TransportResult:
        """Send through a locally attached Meshtastic radio when available, otherwise MQTT."""
        try:
            from services.meshtastic_local_device import local_meshtastic_device

            local_status = local_meshtastic_device.status()
            if local_status.get("connected"):
                local_result = local_meshtastic_device.send_text(envelope.payload, envelope.destination or "")
                if local_result.get("ok"):
                    return TransportResult(True, self.NAME, "Yerel Meshtastic cihazı üzerinden gönderildi")
        except Exception:
            pass
