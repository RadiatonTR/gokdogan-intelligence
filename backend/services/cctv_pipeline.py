...
DB_PATH = runtime_data_dir() / "cctv.db"
...
class BaseCCTVIngestor(ABC):
    ...
    def ingest(self):
        # Ensure the active runtime/test DB and parent directory exist before ingest.
        init_db()
        conn = sqlite3.connect(str(DB_PATH))
        try:
...
def _split_public_camera_setting(name: str, *, limit: int = 32) -> list[str]:
...
def _hostname_resolves_only_public_addresses(hostname: str) -> bool:
...
def _public_camera_catalog_url_allowed(url: str) -> bool:
    candidate = str(url or "").strip()
    parsed = urlparse(candidate)
    if parsed.scheme.lower() != "https" or not parsed.hostname:
        return False
    allowed_hosts = [host.lower().strip().rstrip(".") for host in _split_public_camera_setting("GOKDOGAN_PUBLIC_CAMERA_CATALOG_HOSTS")]
    host = str(parsed.hostname or "").lower().strip().rstrip(".")
    if not allowed_hosts:
        return False
    if not any(host == allowed or host.endswith(f".{allowed}") for allowed in allowed_hosts):
        return False
    return _hostname_resolves_only_public_addresses(host)


def _catalog_row_is_explicitly_public(row: Dict[str, Any]) -> bool:
    value = row.get("public_access_confirmed", row.get("public_access", row.get("is_public")))
    if value is True:
        return True
    return str(value or "").strip().lower() in {"1", "true", "yes", "public", "open", "official_public"}


def _camera_rows_from_authorized_catalog(payload: Any, *, catalog_url: str) -> List[Dict[str, Any]]:
    if isinstance(payload, dict) and str(payload.get("type") or "").lower() == "featurecollection":
        raw_rows = payload.get("features") or []
    elif isinstance(payload, dict):
        raw_rows = payload.get("cameras") or payload.get("items") or payload.get("data") or []
    ...
    cameras: List[Dict[str, Any]] = []
    catalog_tag = hashlib.sha256(catalog_url.encode("utf-8")).hexdigest()[:10]
    for index, raw in enumerate(raw_rows):
        if not isinstance(raw, dict):
            continue
        properties = raw.get("properties") if isinstance(raw.get("properties"), dict) else raw
        if not _catalog_row_is_explicitly_public(properties):
            continue
        geometry = raw.get("geometry") if isinstance(raw.get("geometry"), dict) else {}
        coordinates = geometry.get("coordinates") if isinstance(geometry.get("coordinates"), (list, tuple)) else None
        lat = properties.get("lat", properties.get("latitude"))
        lon = properties.get("lon", properties.get("lng", properties.get("longitude")))
        ...
        raw_media = (
            properties.get("media_url")
            or properties.get("stream_url")
            or properties.get("image_url")
            or properties.get("snapshot_url")
            or ""
        )
        media_url = _ensure_https_url(str(raw_media or "").strip())
        if media_url:
            parsed_media = urlparse(media_url)
            if parsed_media.scheme not in {"http", "https"} or not parsed_media.hostname:
                media_url = ""
        source = str(
            properties.get("source_agency")
            or properties.get("operator")
            or properties.get("agency")
            or properties.get("source")
            or "Authorized Public Camera Catalog"
        ).strip()[:60]
        label = str(
            properties.get("direction_facing")
            or properties.get("name")
            or properties.get("title")
            or properties.get("location")
            or "Public camera"
        ).strip()[:120]
        source_id = str(properties.get("id") or properties.get("camera_id") or properties.get("uuid") or index).strip()
        safe_id = re.sub(r"[^A-Za-z0-9_.:-]+", "_", source_id)[:80] or str(index)
        cameras.append(
            {
                "id": f"pubcat-{catalog_tag}-{safe_id}",
                "source_agency": source,
                "lat": lat_f,
                "lon": lon_f,
                "direction_facing": label,
                "media_url": media_url,
                "media_type": _detect_media_type(media_url) if media_url else "metadata",
                "refresh_rate_seconds": max(10, min(int(properties.get("refresh_rate_seconds") or 60), 3600)),
            }
        )
    return cameras


class AuthorizedPublicCameraCatalogIngestor(BaseCCTVIngestor):
    """Import explicitly-authorized public camera catalogs.

    Catalog URLs must be HTTPS, their hosts must be explicitly allowlisted in
    ``GOKDOGAN_PUBLIC_CAMERA_CATALOG_HOSTS``, and each camera record must carry
    ``public_access_confirmed=true`` (or an equivalent explicit public marker).
    This deliberately cannot discover, brute-force, or proxy private cameras.
    """

    def fetch_data(self) -> List[Dict[str, Any]]:
        urls = _split_public_camera_setting("GOKDOGAN_PUBLIC_CAMERA_CATALOG_URLS", limit=12)
        cameras: List[Dict[str, Any]] = []
        for catalog_url in urls:
            if not _public_camera_catalog_url_allowed(catalog_url):
                logger.warning("Skipping unauthorized/non-public camera catalog URL: %s", catalog_url)
                continue
            try:
                response = fetch_with_curl(catalog_url, timeout=20, headers={"Accept": "application/json"})
                if not response or int(response.status_code or 500) >= 400:
                    logger.warning("Public camera catalog fetch failed: %s HTTP %s", catalog_url, getattr(response, "status_code", "no response"))
                    continue
                cameras.extend(_camera_rows_from_authorized_catalog(response.json(), catalog_url=catalog_url))
            except Exception as exc:
                logger.warning("Public camera catalog ingest failed for %s: %s", catalog_url, exc)
        return cameras

class TurkeyCustomsBorderIngestor(BaseCCTVIngestor):
    """T.C. Ticaret Bakanlığı Trakya public border-gate camera snapshots.

    The ministry page publishes periodically refreshed public images for major
    Trakya crossings. We only ingest image URLs that are explicitly associated
    with a named gate on that public page; no authentication, private feeds, or
    security-camera discovery is attempted.
    """

    PAGE_URL = "https://trakya.ticaret.gov.tr/yayinlar/canli-kameralar"
    GATES = {
        "kapikule": ("Kapıkule Gümrük Kapısı", 41.7167, 26.3572),
        "hamzabeyli": ("Hamzabeyli Gümrük Kapısı", 41.9738, 26.5988),
        "pazarkule": ("Pazarkule Gümrük Kapısı", 41.6694, 26.5268),
        "ipsala": ("İpsala Gümrük Kapısı", 40.9220, 26.3806),
        "derekoy": ("Dereköy Gümrük Kapısı", 41.9688, 27.3484),
    }
        ...
        try:
            ...
            for idx, img in enumerate(soup.find_all("img")):
                ...
                if not src or src.startswith("data:"):
                    continue
                media_url = _ensure_https_url(urljoin(self.PAGE_URL, src))
                if not media_url or media_url in seen:
                    continue
                # Resolve the gate from the most specific context first. This avoids
                # a broad page/body container causing the previous gate heading to
                # leak into the next image.
                matched = None
                direct_context = self._fold(" ".join([
                    str(img.get("alt") or ""),
                    str(img.get("title") or ""),
                    media_url,
                ]))
                for gate_key, gate in self.GATES.items():
                    if gate_key in direct_context:
                        matched = (gate_key, gate)
                        break

                if not matched:
                    # Walk only nearest ancestors, stopping as soon as one uniquely
                    # identifies a gate. Prefer the closest heading text if present.
                    previous_heading = img.find_previous(["h1", "h2", "h3", "h4", "strong", "b"])
                    contexts = []
                    if previous_heading is not None:
                        contexts.append(previous_heading.get_text(" ", strip=True)[:240])
                    ancestor = img.parent
                    for _ in range(3):
                        if ancestor is None:
                            break
                        try:
                            contexts.append(ancestor.get_text(" ", strip=True)[:400])
                        except Exception:
                            pass
                        ancestor = getattr(ancestor, "parent", None)
                    for raw_context in contexts:
                        folded = self._fold(raw_context)
                        hits = [(gate_key, gate) for gate_key, gate in self.GATES.items() if gate_key in folded]
                        if len(hits) == 1:
                            matched = hits[0]
                            break
                if not matched:
                    continue
                gate_key, (label, lat, lon) = matched
                seen.add(media_url)
                cameras.append({
                    "id": f"TRCUSTOMS-{gate_key}-{idx}",
                    "source_agency": "T.C. Ticaret Bakanlığı / Trakya Gümrük",
                    "lat": lat,
                    "lon": lon,
                    "direction_facing": label,
                    "media_url": media_url,
            ...
            logger.info("TurkeyCustomsBorderIngestor: parsed %s public border snapshots", len(cameras))
            return cameras
        except Exception as exc:
            logger.warning("Turkey customs public camera ingest failed: %s", exc)
            return []
...
def scheduled_cctv_ingestors() -> List[tuple["BaseCCTVIngestor", str]]:
    ...
    return [
        ...
        (AuthorizedPublicCameraCatalogIngestor(), "cctv_public_catalog"),
        (TurkeyCustomsBorderIngestor(), "cctv_tr_customs"),
    ]
