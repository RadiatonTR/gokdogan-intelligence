...
def _apply_seismic_threats(suppliers: list[dict[str, Any]], earthquakes: list[dict[str, Any]]) -> None:
    for sup in suppliers:
        ...
        if best:
            ...
            sup["active_threats"].append(f"SİSMİK YAKINLIK (M{mag:.1f})")
    ...
    for sup in suppliers:
        ...
        if count:
            if sup["risk_level"] == "NORMAL":
                sup["risk_level"] = "HIGH"
            sup["active_threats"].append(f"ORMAN YANGINI YAKINLIĞI ({count} sıcak nokta)")
    ...
    for sup in suppliers:
        for event in conflicts:
            ...
            if _distance_km(sup["lat"], sup["lng"], float(lat), float(lng)) < 100:
                sup["risk_level"] = "CRITICAL"
                sup["active_threats"].append("SİLAHLI ÇATIŞMA / TOPLUMSAL OLAY")
                break
