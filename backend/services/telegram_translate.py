...
def telegram_translate_target() -> str:
    raw = str(os.environ.get("TELEGRAM_OSINT_TRANSLATE_TO", "tr")).strip().lower()
    return _LOCALE_TO_GOOGLE.get(raw, raw or "tr")
...
def source_lang_label(code: str | None) -> str:
    ...
    return _SOURCE_LANG_LABELS.get(raw, raw.upper() if raw else "Bilinmiyor")
...
def _google_translate(clean: str, target: str, source: str | None = None) -> tuple[str, str]:
    ...
    resp = requests.get(
        url,
        timeout=8,
        headers={"User-Agent": "Mozilla/5.0 (compatible; Gokdogan-Translate/1.0)"},
    )
