...
def ais_proxy_status() -> dict:
    ...
    # Preserve the documented pre-start contract: before the AIS subsystem has
    # emitted status, spawned the proxy, or received a vessel message, health
    # reports should remain empty rather than looking like an active-but-dead
    # connection. Once the proxy has actually started, expose connectivity
    # telemetry alongside any status marker.
    if last <= 0 and spawns <= 0:
        return status
...
def _ais_stream_loop():
    ...
    while _ws_running:
        try:
            ...
            node_bin = os.environ.get("SB_NODE_BIN", "").strip() or "node"
