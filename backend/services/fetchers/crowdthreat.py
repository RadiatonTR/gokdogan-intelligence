...
def crowdthreat_fetch_enabled() -> bool:
    """Enable the public CrowdThreat feed in the Gokdogan live-data profile.

    Generic upstream builds remain opt-in via ``CROWDTHREAT_ENABLED``; the
    packaged Gokdogan desktop already opts into public live data with
    ``GOKDOGAN_LIVE_DATA=true``.
    """
    truthy = {"1", "true", "yes", "on"}
    explicit = str(os.environ.get("CROWDTHREAT_ENABLED", "")).strip().lower()
    if explicit:
        return explicit in truthy
    return str(os.environ.get("GOKDOGAN_LIVE_DATA", "")).strip().lower() in truthy
