...
def _report_failure(function_name: str, exc: BaseException) -> None:
    try:
        from services.fetchers._store import mark_source_failure
        layer = function_name.removeprefix("fetch_").replace("_", "-")
        mark_source_failure(layer, exc)
    except Exception:
        pass
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(1 + max_retries):
                try:
                    ...
                    _report_failure(func.__name__, exc)
                    raise
