...
from services.fetchers.public_events import fetch_global_disasters, fetch_border_status  # noqa: F401
...
# ---------------------------------------------------------------------------
def _runtime_offline_mode() -> bool:
    """Read the analyst runtime preference without making network I/O.

    Cold-start cache loading remains available while offline; only periodic
    remote refresh tiers are skipped. The helper fails open if the optional
    Intelligence Core is unavailable so legacy startup behavior is preserved.
    """
    try:
        from services.intelligence_core import get_intelligence_core
        prefs = get_intelligence_core().store.get_setting("runtime_preferences", {}) or {}
        return bool(prefs.get("offline_mode"))
    except Exception:
        return False
...
def update_fast_data():
    ...
    if _runtime_offline_mode():
        logger.info("Fast-tier remote refresh skipped: Intelligence Core offline mode is enabled")
        return
...
def update_slow_data():
    ...
    if _runtime_offline_mode():
        logger.info("Slow-tier remote refresh skipped: Intelligence Core offline mode is enabled")
        return
...
_scheduler = None
_STARTUP_CCTV_INGEST_DELAY_S = int(os.environ.get("SHADOWBROKER_STARTUP_CCTV_INGEST_DELAY_S", "10"))
