...
from services.desktop_safety import active_recon_allowed
...
def run_osint_sweep(args: dict[str, Any]) -> dict[str, Any]:
    """Run subnet device discovery (Shodan InternetDB proxy). Requires explicit operator opt-in."""
    if not active_recon_allowed():
        raise ValueError(
            "Active subnet discovery is disabled. Set SB_ALLOW_ACTIVE_RECON=true only for authorized assets."
        )
    ip = _require_str(args, "ip", "query", "value")
    cidr = args.get("cidr", 24)
    try:
