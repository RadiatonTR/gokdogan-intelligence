...
def lookup_threats(query: str | None = None) -> dict[str, Any]:
    ...
    if query:
        if _IPV4_RE.match(query):
            ...
            abuse_key = str(os.getenv("ABUSEIPDB_API_KEY", "") or "").strip()
            if abuse_key:
                abuse = _json_get(
                    f"https://api.abuseipdb.com/api/v2/check?ipAddress={quote(query)}&maxAgeInDays=90&verbose=",
                    timeout=6,
                    headers={"Accept": "application/json", "Key": abuse_key},
                )
                data = abuse.get("data") if isinstance(abuse, dict) else None
                if isinstance(data, dict):
                    results["abuseipdb"] = {
    ...
    abuse_score = int((results.get("abuseipdb") or {}).get("abuse_confidence_score", 0) or 0)
    results["threat_level"] = (
        "HIGH" if pulse_count > 5 or abuse_score >= 75
        else "MEDIUM" if pulse_count > 0 or abuse_score >= 25
        else "LOW"
    )
    return results
