...
# - Is persisted to the per-user runtime data directory so it survives
#   restarts without modifying the source/release tree. `SB_DATA_DIR` can
#   explicitly redirect this state for managed/Docker deployments.
...
_OPERATOR_HANDLE_FILE = runtime_data_dir() / "operator_handle.json"
...
def _load_persisted_operator_handle() -> str:
    ...
    Reads the per-user runtime ``operator_handle.json`` if it exists. Any read
