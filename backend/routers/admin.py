...
@router.get("/api/settings/api-keys/diagnostics", dependencies=[Depends(require_local_operator)])
@limiter.limit("30/minute")
async def api_get_key_diagnostics(request: Request):
    from services.api_settings import get_api_key_diagnostics
    return get_api_key_diagnostics()
...
@router.put("/api/settings/api-keys/runtime", dependencies=[Depends(require_local_operator)])
@limiter.limit("20/minute")
async def api_apply_keys_runtime(request: Request):
    """Hot-apply native-vault secrets without persisting plaintext to backend files."""
    from services.api_settings import apply_api_keys_runtime
    body = await request.json()
    if not isinstance(body, dict):
        return Response(
            content=json_mod.dumps({"ok": False, "detail": "Expected a JSON object."}),
    ...
    result = apply_api_keys_runtime({str(k): str(v) for k, v in body.items()})
    if result.get("ok"):
        return result
    return Response(
        content=json_mod.dumps(result),
...
@router.get("/api/settings/integrations/readiness", dependencies=[Depends(require_local_operator)])
@limiter.limit("30/minute")
async def api_integration_readiness(request: Request):
    from services.integration_readiness import integration_readiness_snapshot
    return integration_readiness_snapshot()


@router.post("/api/settings/integrations/{integration_id}/refresh", dependencies=[Depends(require_local_operator)])
@limiter.limit("20/minute")
async def api_refresh_integration(integration_id: str, request: Request):
    ...
    return Response(
        content=json_mod.dumps(result),
        status_code=409 if result.get("detail") in {"needs_key", "partial_config"} else 400,
        media_type="application/json",
    )
