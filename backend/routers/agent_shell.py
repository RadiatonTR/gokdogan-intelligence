...
if sys.platform != "win32":
    ...
    import select
    import termios
else:  # Keep the module importable on native Windows.
...
from services.desktop_safety import agent_shell_allowed
...
def _require_agent_shell_enabled() -> None:
    if not agent_shell_allowed():
        raise HTTPException(
            status_code=403,
            detail=(
                "Host shell is disabled by default in the desktop runtime. "
                "Set SB_ALLOW_AGENT_SHELL=true only when local shell access is explicitly required."
            ),
        )
...
@router.get("/api/agent-shell/settings", dependencies=[Depends(require_local_operator), Depends(_require_agent_shell_enabled)])
async def read_agent_shell_settings() -> dict[str, Any]:
    return get_agent_shell_settings()


@router.post("/api/agent-shell/ws-token", dependencies=[Depends(require_local_operator), Depends(_require_agent_shell_enabled)])
async def mint_agent_shell_ws_token_route() -> dict[str, Any]:
    token, expires_in = mint_agent_shell_ws_token()
    return {"token": token, "expires_in": expires_in}


@router.put("/api/agent-shell/settings", dependencies=[Depends(require_local_operator), Depends(_require_agent_shell_enabled)])
...
async def agent_shell_websocket(
    ...
    await ws.accept()
    if not agent_shell_allowed():
        await ws.close(code=4403, reason="host shell disabled by desktop safety policy")
        return
    try:
