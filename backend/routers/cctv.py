...
def _cctv_host_allowed(hostname) -> bool:
    from services.cctv_public_policy import configured_public_camera_media_host_allowed

    host = str(hostname or "").strip().lower().rstrip(".")
    if not host:
        return False
    for allowed in _CCTV_PROXY_ALLOWED_HOSTS:
        normalized = str(allowed or "").strip().lower().rstrip(".")
        if host == normalized or host.endswith(f".{normalized}"):
            return True
    return configured_public_camera_media_host_allowed(host)
