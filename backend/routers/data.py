...
@router.get("/api/sigint/meshtastic/device")
@limiter.limit("120/minute")
async def meshtastic_local_device_status(request: Request):  # noqa: ARG001
    from services.meshtastic_local_device import local_meshtastic_device

    return local_meshtastic_device.status()


@router.post("/api/sigint/meshtastic/device/connect", dependencies=[Depends(require_local_operator)])
@limiter.limit("20/minute")
async def meshtastic_local_device_connect(request: Request):
    from services.meshtastic_local_device import local_meshtastic_device

    body = await request.json()
    return local_meshtastic_device.connect(
        mode=str(body.get("mode") or "serial"),
        target=str(body.get("target") or ""),
    )


@router.post("/api/sigint/meshtastic/device/disconnect", dependencies=[Depends(require_local_operator)])
@limiter.limit("20/minute")
async def meshtastic_local_device_disconnect(request: Request):  # noqa: ARG001
    from services.meshtastic_local_device import local_meshtastic_device

    return local_meshtastic_device.disconnect()
...
async def live_data_slow(
    ...
    def _build() -> dict:
        d = get_latest_data_subset_refs(
            "last_updated", "news", "stocks", "financial_source", "financial_updated_at", "financial", "crypto", "fx", "metals", "indices", "oil", "weather", "traffic",
            ...
            "crowdthreat", "threat_level", "trending_markets", "road_corridor_trends", "global_disasters", "border_status",
        ...
        payload = {
            ...
            "financial_updated_at": d.get("financial_updated_at", ""),
            ...
            "crowdthreat": (d.get("crowdthreat") or []) if active_layers.get("crowdthreat", True) else [],
            "global_disasters": d.get("global_disasters") or {"events": [], "sources": {}, "updated_at": None, "total": 0},
            "border_status": d.get("border_status") or {"crossings": [], "sources": {}, "updated_at": None, "total": 0},
