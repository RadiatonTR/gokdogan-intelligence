...
def infonet_status() -> dict[str, Any]:
    ...
    # Privacy primitives are experimental scaffolding. R7 keeps them outside
    # the production intelligence path and disabled by default; status remains
    # truthful even when an operator explicitly enables the lab surface.
    experimental_privacy_enabled = str(os.environ.get("SB_ENABLE_EXPERIMENTAL_PRIVACY", "false")).strip().lower() in {"1", "true", "yes", "on"}
    privacy = {
    ...
    return {
        ...
        "experimental_privacy_enabled": experimental_privacy_enabled,
        "experimental_privacy_warning": "Unaudited scaffolding; not production cryptography or a privacy guarantee.",
