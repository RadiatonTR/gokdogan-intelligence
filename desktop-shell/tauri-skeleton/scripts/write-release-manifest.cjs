...
function hashInput(relativePath) {
  ...
  return { path: relativePath.replaceAll(path.sep, '/'), sha256: sha256File(fullPath) };
}

function releaseMetadata(wroteUpdaterManifest) {
  let config = {};
  try { config = JSON.parse(fs.readFileSync(tauriConfigPath, 'utf8')); } catch {}
  const sourceInputs = [
    'uv.lock',
    'frontend/package-lock.json',
    'desktop-shell/package-lock.json',
    'desktop-shell/tauri-skeleton/src-tauri/Cargo.lock',
  ].map(hashInput).filter(Boolean);
  return {
    product: String(config.productName || 'Gokdogan Intelligence Desktop'),
    version: readReleaseVersion(),
    revision: readTextIfExists(path.join(repoRoot, 'WINDOWS-DESKTOP-BUILD-REVISION.txt')) || process.env.SB_BUILD_REVISION || 'unknown',
    identifier: String(config.identifier || ''),
    built_at: new Date().toISOString(),
    platform: process.platform,
    arch: process.arch,
    node: process.version,
    updater_manifest_written: Boolean(wroteUpdaterManifest),
...
fs.writeFileSync(
  ...
  `${JSON.stringify({ metadata: releaseMetadata(wroteUpdaterManifest), artifacts: manifest }, null, 2)}\n`,
);
