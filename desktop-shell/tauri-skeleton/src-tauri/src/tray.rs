...
//!   the menu; "Gokdogan’ı Göster" restores the window. "Quit" exits fully.
...
pub fn setup_tray(app: &AppHandle) -> Result<TrayIcon, String> {
    let show_item = MenuItem::with_id(app, MENU_ID_SHOW, "Gokdogan’ı Göster", true, None::<&str>)
    ...
    let quit_item = MenuItem::with_id(app, MENU_ID_QUIT, "Gokdogan’dan Çık", true, None::<&str>)
...
///   background. The user can restore via tray menu or quit via "Gokdogan’dan Çık".
