...
async fn api_proxy(
    ...
    let mut builder = state.client.request(req_method.clone(), &target)
        // Keep the loopback proxy deterministic: the browser may advertise gzip/br,
        // while this reqwest profile intentionally has no decompression feature.
        // Asking the FastAPI backend for identity bytes prevents compressed JSON
        // from being forwarded after Content-Encoding has been stripped.
