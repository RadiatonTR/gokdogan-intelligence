...
pub struct LoadOutcome<T> {
    pub value: T,
    // Production callers consume the migrated value and status side effects;
    // the flag is retained for the migration regression tests and diagnostics.
    #[allow(dead_code)]
    pub migrated: bool,
}
...
fn atomic_write_bytes(target: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    // Windows does not allow std::fs::rename() to replace an existing target.
    // Vault/state files are updated repeatedly, so rotate the previous file to
    // a same-directory backup first and restore it if promotion fails.
    let backup_path = target.with_extension("replace-backup");
    if backup_path.exists() {
        fs::remove_file(&backup_path)
            .map_err(|e| format!("native_local_custody_backup_cleanup_failed:{e}"))?;
    }
    let had_target = target.exists();
    if had_target {
        fs::rename(target, &backup_path)
            .map_err(|e| format!("native_local_custody_backup_rotate_failed:{e}"))?;
    }
    if let Err(e) = fs::rename(&tmp_path, target) {
        if had_target && backup_path.exists() {
            let _ = fs::rename(&backup_path, target);
        }
        return Err(format!("native_local_custody_rename_failed:{e}"));
    }
    if backup_path.exists() {
