...
pub fn is_loopback_origin(raw: &str) -> bool {
    // Parse the URL rather than manually splitting authority text. This avoids
    // accepting look-alike/userinfo forms and, importantly, prevents non-HTTP
    // schemes (for example file:// or javascript:) from being treated as a
    // browser companion origin merely because they mention localhost.
...
mod tests {
    ...
    #[test]
    fn loopback_rejects_non_http_schemes_and_lookalikes() {
        assert!(!is_loopback_origin("file://localhost/C:/secret.txt"));
        assert!(!is_loopback_origin("javascript://localhost/alert(1)"));
        assert!(!is_loopback_origin("http://localhost.evil.example:3000"));
        assert!(!is_loopback_origin("http://127.0.0.1@evil.example:3000"));
    }

    #[test]
    fn loopback_accepts_case_insensitive_localhost_and_ipv6() {
