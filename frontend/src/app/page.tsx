...
import WeatherTrafficPanel from '@/components/WeatherTrafficPanel';
import PublicIntelPanel from '@/components/PublicIntelPanel';
import DiplomacyFeed from '@/components/DiplomacyFeed';
...
import { useDataPolling, LAYER_TOGGLE_EVENT, forceRefreshLiveData } from '@/hooks/useDataPolling';
...
import { useIntelligenceAlertNotifications } from '@/hooks/useIntelligenceAlertNotifications';
...
const IntelligenceCenterPanel = dynamic(() => import('@/components/IntelligenceCenterPanel'), { ssr: false });
...
export default function Dashboard() {
  ...
  useDataPolling();
  useIntelligenceAlertNotifications();
  ...
  const marketData = useDataKeys(['stocks', 'crypto', 'fx', 'metals', 'indices', 'oil', 'financial_source', 'financial_updated_at', 'financial', 'unusual_whales'] as const);
  ...
  const [intelligenceCenterOpen, setIntelligenceCenterOpen] = useState(false);
  const [intelligenceInitialTab, setIntelligenceInitialTab] = useState<'overview' | 'search'>('overview');
  ...
  useKeyboardShortcuts({
    ...
    openIntelligenceSearch: () => {
      setIntelligenceInitialTab('search');
      setIntelligenceCenterOpen(true);
    },
  });
  ...
  return (
    <>
      <main className="fixed inset-0 w-full h-full bg-[var(--bg-primary)] overflow-hidden font-sans">
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                >
                  G O K <span className="text-cyan-400">D O G A N</span>
                </h1>
            ...
            >
              ...
              <button
                onClick={() => { setIntelligenceInitialTab('overview'); setIntelligenceCenterOpen(true); }}
                className="w-full flex items-center justify-between px-3 py-2 border border-cyan-800/50 bg-cyan-950/20 hover:bg-cyan-950/35 text-[9px] font-mono tracking-[0.18em] text-cyan-300 transition-colors"
                title="İstihbarat merkezini aç: kaynaklar, vakalar, uyarılar ve tanılama"
              >
                <span>İSTİHBARAT MERKEZİ</span>
                <span className="text-emerald-400">● YEREL</span>
              </button>

              <button
                onClick={() => setMarketsOpen(true)}
                className="w-full flex items-center justify-between px-3 py-2 border border-emerald-800/50 bg-emerald-950/15 hover:bg-emerald-950/30 text-[9px] font-mono tracking-[0.18em] text-emerald-300 transition-colors"
                title="Döviz, değerli maden, kripto, enerji ve dünya endekslerini aç"
              >
                <span>PİYASA MERKEZİ</span>
                <span className="text-cyan-400">● CANLI</span>
              </button>

              <ErrorBoundary name="WeatherTrafficPanel">
                <WeatherTrafficPanel viewBoundsRef={viewBoundsRef} onOpenSettings={() => setSettingsOpen(true)} />
              </ErrorBoundary>

              <ErrorBoundary name="PublicIntelPanel">
                <PublicIntelPanel onLocate={(lat, lng) => setFlyToLocation({ lat, lng, ts: Date.now() })} />
              </ErrorBoundary>
              ...
              <div className={`${rightFocusedPanel ? 'hidden' : ''}`}>
                <ErrorBoundary name="DiplomacyFeed">
                  <DiplomacyFeed />
                </ErrorBoundary>
              </div>
        ...
        {/* INTELLIGENCE CORE */}
        {intelligenceCenterOpen && (
          <ErrorBoundary name="IntelligenceCenterPanel">
            <IntelligenceCenterPanel initialTab={intelligenceInitialTab} onClose={() => setIntelligenceCenterOpen(false)} />
          </ErrorBoundary>
        )}
        ...
        {marketsOpen && (
          <div className="fixed inset-0 z-[9500] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={() => setMarketsOpen(false)}>
            <div className="w-full max-w-5xl max-h-[88vh] overflow-y-auto styled-scrollbar shadow-[0_0_40px_rgba(6,182,212,0.22)]" onMouseDown={(event) => event.stopPropagation()}>
              <div className="flex items-center justify-between border border-cyan-900/50 border-b-0 bg-[#080d10]/95 px-4 py-2 font-mono">
                <div>
                  <div className="text-[11px] tracking-[0.2em] text-cyan-300 font-bold">GOKDOGAN PİYASA MERKEZİ</div>
                  <div className="text-[8px] text-[var(--text-muted)] mt-0.5">Döviz · değerli madenler · kripto · enerji · dünya endeksleri</div>
                </div>
                <button onClick={() => setMarketsOpen(false)} className="px-3 py-1 border border-cyan-900/50 text-[9px] text-cyan-400 hover:text-white">KAPAT</button>
              </div>
              <MarketsPanel data={marketData as import('@/types/dashboard').DashboardData} focused />
            </div>
          </div>
        )}
        ...
        {backendStatus === 'disconnected' && (
          <div className="absolute top-0 left-0 right-0 z-[9000] flex items-center justify-center gap-3 py-2 bg-red-950/90 border-b border-red-500/40 backdrop-blur-sm">
            <span className="text-[10px] font-mono tracking-widest text-red-400">
              {t('backend.offline')}
            </span>
            <button
              type="button"
              onClick={() => void forceRefreshLiveData()}
              className="px-2 py-1 border border-red-500/40 text-[9px] font-mono tracking-widest text-red-200 hover:bg-red-500/10"
            >
              YENİDEN BAĞLAN
            </button>
          </div>
