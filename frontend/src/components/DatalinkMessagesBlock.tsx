...
export default function DatalinkMessagesBlock({
    ...
    async (opts?: { showLoading?: boolean }) => {
      ...
      try {
        ...
        if (opts?.showLoading) {
          setConfigured(null);
          setMessages([]);
          setLoadError('ACARS önbelleğine ulaşılamadı. Kısa bir süre sonra yeniden deneyin.');
        }
      } finally {
  ...
  if (loading) {
    return (
      <div className="border-b border-[var(--border-primary)] pb-2">
        ...
        <span className="text-[10px] font-mono text-[var(--text-muted)]">ACARS önbelleği yükleniyor…</span>
      </div>
    );
  }
    ...
    return (
      <div className="border-b border-[var(--border-primary)] pb-2">
        ...
        <p className="text-[10px] font-mono text-amber-400/90 leading-relaxed">
          {hint || 'Uçak dosyalarında ACARS veri bağlantısını etkinleştirmek için Ayarlar → API Anahtarları bölümüne Airframes API anahtarınızı ekleyin.'}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="border-b border-[var(--border-primary)] pb-2">
        ...
        <p className="text-[10px] font-mono text-[var(--text-muted)]">
          {priorityScanning
            ? 'Bu hava aracı için öncelikli tarama sıraya alındı (~2 sn)…'
            : 'Bu hava aracı için yakın tarihli ACARS/VDL mesajı yok.'}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="border-b border-[var(--border-primary)] pb-2">
      <span className="text-[var(--text-muted)] text-[10px] block mb-1">DATALINK (AIRFRAMES)</span>
      {priorityScanning ? (
        <p className="text-[10px] font-mono text-cyan-500/70 mb-1">Bu hava aracı yenileniyor…</p>
      ) : null}
