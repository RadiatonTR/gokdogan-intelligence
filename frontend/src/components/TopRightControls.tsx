...
const CUSTOM_DESKTOP_BUILD = process.env.NEXT_PUBLIC_SB_CUSTOM_DESKTOP === '1';
...
const DESKTOP_UPDATER_DETAIL =
  'This packaged desktop app can install the signed update in place. It will kurulum tamamlandıktan sonra Gokdogan’ı yeniden başlatır.';
...
export default function TopRightControls({
  ...
  const setNodeEnabled = async (enabled: boolean) => {
    ...
    try {
      ...
      if (enabled) {
        ...
        // Participant-node sync is a public/local mesh function and must not be
        // blocked by optional Wormhole/Tor transport. Warm the private transport
        // opportunistically in the background; node activation continues even
        // when Tor/Reticulum is unavailable.
        setActivatingPhase('peers');
        void startTorHiddenService().catch(() => null);
      }
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      if (CUSTOM_DESKTOP_BUILD && runtime === 'desktop_packaged' && !desktopContext?.updater_enabled) {
        setUpdateDetail('Custom Windows desktop build: upstream automatic updates are disabled so local modifications are not overwritten. Rebuild this bundle to update it.');
      ...
      try {
        ...
        setErrorMessage(
          message === 'desktop_update_installed_restart_required'
            ? 'Update installed. Uygulamayı tamamlamak için Gokdogan’ı yeniden başlatın.'
  ...
  const nodeModeLabel = nodeMode === 'PARTICIPANT' ? 'KATILIMCI' : nodeMode;
  ...
  const nodeTitle = !nodeEnabled
    ? `${nodeModeLabel} düğüm • kapalı`
    : bootstrapFailed
      ? `${nodeModeLabel} düğüm • başlangıç uyarısı`
      : `${nodeModeLabel} düğüm • ${syncOutcome.toLowerCase()}`;
  const closeLauncher = () => {
    stopActivatingPolls();
    setLauncherOpen(false);
    setNodeStep('prompt');
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-cyan-900/30">
                <div>
                  ...
                  <div className="mt-1 text-[9px] font-mono text-[var(--text-muted)]">
                    {nodeModeLabel} • {syncOutcome} • katılımcı düğüm eşitlemesi Wormhole gerektirmez
                  </div>
                </div>
                <button
                  type="button"
