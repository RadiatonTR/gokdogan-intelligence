...
const API_GUIDES = [
  {
    ...
    required: true,
    description:
      'Küresel ADS-B kapsamasıyla sivil uçuş takibi ve güncel uçak konumları.',
    steps: [
      'opensky-network.org üzerinde ücretsiz hesap oluşturun',
      'Panel → OAuth → İstemci Oluştur bölümüne gidin',
      'İstemci Kimliği ve İstemci Gizli Anahtarını kopyalayın',
      'İkisini de Hızlı Yerel Kurulum veya Ayarlar → API Anahtarları bölümüne yapıştırın',
    ],
  ...
  },
  {
    ...
    required: true,
    description: 'AIS (Otomatik Tanımlama Sistemi) üzerinden güncel gemi takibi.',
    steps: [
      'aisstream.io üzerinde kayıt olun',
      'API Anahtarları sayfanıza gidin',
      'Yeni bir API anahtarı oluşturun',
      'Anahtarı Hızlı Yerel Kurulum veya Ayarlar → API Anahtarları bölümüne yapıştırın',
    ],
  ...
  },
  {
    ...
    required: false,
    description:
      'Balıkçılık etkinliği harita katmanı için gemi olayları. Denizcilik OSINT zenginleştirmesi için isteğe bağlıdır.',
    steps: [
      'globalfishingwatch.org üzerinde ücretsiz hesap oluşturun',
      'API bölümünü açıp bir erişim belirteci oluşturun',
      'Belirteci Hızlı Yerel Kurulum veya Ayarlar → API Anahtarları → Denizcilik bölümüne yapıştırın',
    ],
...
const OnboardingModal = React.memo(function OnboardingModal({
  ...
  const [setupMsg, setSetupMsg] = useState<{ type: 'ok' | 'warn' | 'err'; text: string } | null>(null);
  ...
  const saveSetupKeys = async () => {
    ...
    if (!Object.keys(payload).length) {
      setSetupMsg({ type: 'err', text: 'Önce en az bir API anahtarı girin.' });
      return;
    }
    setSetupSaving(true);
    setSetupMsg(null);
    try {
      const saved = await saveApiKeysResilient(
      ...
      setSetupMsg({ type: saved.warning ? 'warn' : 'ok', text: saved.warning || (saved.hotApplied ? 'API anahtarları kaydedildi ve hemen etkinleştirildi.' : 'API anahtarları kaydedildi. Etkinleşmesi için uygulamayı yeniden başlatın.') });
    } catch (error) {
      const rawMessage = error instanceof Error ? error.message : typeof error === 'string' ? error : '';
      setSetupMsg({
        type: 'err',
        text: rawMessage === 'native_tauri_api_unavailable'
          ? 'Masaüstü güvenli kasa bağlantısı hazır değil. Uygulamayı yeniden başlatın; sorun sürerse güncel Gökdoğan paketini kurun.'
          : rawMessage || 'API anahtarları kaydedilemedi.',
      });
    } finally {
  ...
  const agentSnippet = [
    ...
    agentSecret ? `SHADOWBROKER_HMAC_SECRET=${agentSecret}` : 'SHADOWBROKER_HMAC_SECRET=<generate in Gökdoğan>',
  ...
  const fetchAgentConnectInfo = async (reveal = true) => {
    ...
    try {
      ...
      if (!metaRes.ok || metaData?.ok === false) {
        throw new Error(metaData?.detail || 'YZ ajan kimlik bilgileri hazırlanamadı.');
      }
      ...
      if (!secretSet) {
        ...
        if (!bootRes.ok || bootData?.ok === false) {
          throw new Error(bootData?.detail || 'YZ ajan kimlik bilgileri oluşturulamadı.');
        }
      ...
      if (reveal && secretSet) {
        ...
        if (!revealRes.ok || revealData?.ok === false) {
          throw new Error(revealData?.detail || 'YZ ajan kimlik bilgileri gösterilemedi.');
        }
        setAgentSecret(revealData.hmac_secret || '');
      } else {
        setAgentSecret(metaData.masked_hmac_secret || '');
      }
      setAgentMsg({ type: 'ok', text: 'YZ ajan anahtarı hazır. Yerel veya uzak ajan çalışma ortamınıza kopyalayın.' });
    } catch (error) {
      setAgentMsg({
        type: 'err',
        text: error instanceof Error ? error.message : 'YZ ajan kimlik bilgileri hazırlanamadı.',
      });
    } finally {
  ...
  const saveAgentTier = async (tier: 'restricted' | 'full') => {
    ...
    try {
      ...
      if (!res.ok || data?.ok === false) {
        throw new Error(data?.detail || 'YZ ajan erişim düzeyi güncellenemedi.');
      }
      setAgentMsg({
        type: 'ok',
        text: tier === 'full'
          ? 'Tam erişim kaydedildi. Ajan kimliği doğrulandığında panele yazabilir.'
          : 'Kısıtlı erişim kaydedildi. Ajan telemetriyi okuyabilir ancak yazamaz.',
      });
    } catch (error) {
      setAgentMsg({
        type: 'err',
        text: error instanceof Error ? error.message : 'YZ ajan erişim düzeyi güncellenemedi.',
      });
    }
  };
  ...
  const prepareTorAgentAddress = async () => {
    ...
    try {
      ...
      setAgentMsg({
        type: 'ok',
        text: 'Tor is ready. The remote agent link is private to your yerel Gokdogan düğümü.',
      });
    } catch (error) {
      setAgentMsg({
        type: 'err',
        text:
          error instanceof Error
            ? error.message
            : 'Gokdogan Tor’u otomatik kuramadı veya başlatamadı. Ağ erişimini kontrol edip tekrar deneyin.',
      });
    } finally {
            ...
            {step === 1 && (
              <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <p className="text-sm text-green-300 font-mono font-bold flex items-center gap-2">
                        <Shield size={14} /> Salt Okunur
                      </p>
                      <p className="text-[10px] text-[var(--text-muted)] font-mono mt-2">
                        Canlı telemetriyi görebilir ancak hiçbir şeyi değiştiremez
                      </p>
                    </button>
                    <button
                    ...
                    >
                      <p className="text-sm text-amber-300 font-mono font-bold flex items-center gap-2">
                        <Network size={14} /> Tam Erişim
                      </p>
                      <p className="text-[10px] text-[var(--text-muted)] font-mono mt-2">
                        İşaret koyabilir, katman oluşturabilir ve görüntüleme işlemlerini tetikleyebilir
                      </p>
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <div>
                      <p className="text-[11px] text-violet-300 font-mono font-bold tracking-widest">
                        ADIM 3 - BUNU AJANINA KOPYALA
                      </p>
                      <p className="text-[10px] text-[var(--text-muted)] font-mono mt-1">
                        Yerel bir anahtar oluşturun; ardından bu değişkenleri OpenClaw, Hermes veya başka bir HMAC ajanına kopyalayın.
                      </p>
                    </div>
                    <button
                    ...
                    >
                      {agentLoading ? 'OLUŞTURULUYOR...' : 'OLUŞTUR'}
                    </button>
                  </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-[11px] text-violet-200 font-mono font-bold tracking-widest">
                            UZAK AJANLAR İÇİN TOR GEREKLİ
                          </p>
                          <p className="text-[10px] text-[var(--text-muted)] font-mono mt-1 leading-relaxed">
                            Gokdogan Tor’u yerelde kuracak veya kullanacak, ardından bu arka uç için özel bir .onion bağlantısı oluşturacak.
                          </p>
                        </div>
                        <button
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {agentCopied ? 'KOPYALANDI' : 'KOPYALA'}
                    </button>
                  </div>
                ...
                <p className="text-[11px] text-orange-300/80 font-mono leading-relaxed">
                  Uzak ajan erişimi Tor üzerinden imzalı HTTP API kullanır. Wormhole kullanılabildiğinde aynı Tor/Arti taşıma hattını kullanır; MLS tabanlı yerel ajan taşıması henüz planlama aşamasındadır.
                </p>
              </div>
            )}
                ...
                <div className="bg-yellow-950/20 border border-yellow-500/20 p-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--text-secondary)] font-mono leading-relaxed">
                        OpenSky Network ve AIS Stream, Gokdogan’ın canlı uçuş ve gemi verisini etkinleştiren temel kaynaklardır.
                        Global Fishing Watch balıkçılık etkinliği katmanını açar. Anahtarları aşağıya bir kez girin veya daha sonra Ayarlar bölümünü kullanın;
                        gizli değerler yerel Windows kasasında tutulur ve tarayıcıya geri gösterilmez.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-cyan-900/40 bg-cyan-950/10 p-4 space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] font-mono leading-relaxed mt-1">
                      Anahtarları buraya bir kez yapıştırın. Gokdogan bunları yerel güvenli kasada saklar ve gizli değerleri
                      arayüzde hiçbir zaman tekrar göstermez.
                    </p>
                  </div>
                  {[
                  ...
                  {setupMsg && (
                    <p
                      className={`text-sm font-mono ${
                        setupMsg.type === 'ok' ? 'text-green-300' : setupMsg.type === 'warn' ? 'text-amber-300' : 'text-red-300'
                      }`}
                    >
                      {setupMsg.text}
                    </p>
                  ...
                  >
                    {setupSaving ? 'KAYDEDİLİYOR...' : 'ANAHTARLARI YEREL KASAYA KAYDET'}
                  </button>
                </div>
