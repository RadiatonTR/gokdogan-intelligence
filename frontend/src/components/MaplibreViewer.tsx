...
import TrafficRasterOverlay from '@/components/map/TrafficRasterOverlay';
...
import { useMovingEntityBudget } from '@/components/map/hooks/useMovingEntityBudget';
...
const MaplibreViewer = ({
  ...
  const toggle3DView = useCallback(() => {
    ...
    map.easeTo({
      pitch: next ? 58 : 0,
      bearing: next ? map.getBearing() || -18 : 0,
      duration: 700,
    });
  }, [is3DView]);
  ...
  const enabledMovingLayers = [activeLayers.flights, activeLayers.private, activeLayers.jets, activeLayers.military, activeLayers.tracked, shipsLayerEnabled, sigintLayerEnabled].filter(Boolean).length;
  const { limit: budgetMoving } = useMovingEntityBudget(enabledMovingLayers);

  const dynamicCommercialFlights = activeLayers.flights ? budgetMoving(data?.commercial_flights) : undefined;
  const dynamicPrivateFlights = activeLayers.private ? budgetMoving(data?.private_flights) : undefined;
  const dynamicPrivateJets = activeLayers.jets ? budgetMoving(data?.private_jets) : undefined;
  const dynamicMilitaryFlights = activeLayers.military ? budgetMoving(data?.military_flights) : undefined;
  const dynamicTrackedFlights = activeLayers.tracked ? budgetMoving(data?.tracked_flights) : undefined;
  const dynamicShips = shipsLayerEnabled ? budgetMoving(data?.ships) : undefined;
  const dynamicSigint = sigintLayerEnabled ? budgetMoving(data?.sigint) : undefined;
  ...
  const staticGlobalDisasters = globalIncidentsEnabled ? data?.global_disasters?.events : undefined;
  ...
  const staticMapLayers = useStaticMapLayersWorker(
    {
      ...
      gdelt: staticGdelt,
      globalDisasters: staticGlobalDisasters,
  ...
  // SAR anomalies/AOI watchboxes use the dedicated /api/sar/aois endpoint and imperative sources.
  ...
  const uavGeoJSON = useMemo(
    () => (activeLayers.uav ? buildUavGeoJSON(data?.uavs, inView) : null),
    [activeLayers.uav, data?.uavs, inView],
  );
  ...
  return (
    <div
      className={`relative h-full w-full z-0 isolate ${selectedEntity && ['region_dossier', 'gdelt', 'global_disaster', 'liveuamap', 'news', 'telegram_osint', 'gt_risk'].includes(selectedEntity.type) ? 'map-focus-active' : ''}`}
      style={pinPlacementMode || sarAoiDropMode ? { cursor: 'crosshair' } : undefined}
    >
      <button
        type="button"
        onClick={toggle3DView}
        className={`absolute left-3 top-3 z-30 rounded border px-2.5 py-1.5 text-[10px] font-bold tracking-widest backdrop-blur-sm transition-colors ${
          is3DView
            ? 'border-cyan-300 bg-cyan-950/90 text-cyan-200'
            : 'border-cyan-800/70 bg-black/75 text-cyan-500 hover:text-cyan-200'
        }`}
        title="Haritayı eğimli üç boyutlu görünüm ile göster"
        aria-pressed={is3DView}
      >
        {is3DView ? '3B GÖRÜNÜM AÇIK' : '3B GÖRÜNÜM'}
      </button>
      <Map
        ref={mapRef}
        reuseMaps
      ...
      >
        ...
        <WeatherRadarOverlay enabled={activeLayers.weather_radar} weather={data?.weather} />
        <TrafficRasterOverlay enabled={activeLayers.road_traffic} />
        ...
        <Source id="gdelt" type="geojson" data={EMPTY_FC}>
          <Layer
            ...
            paint={{
              'circle-radius': 5,
              'circle-color': ['match', ['get', 'type'], 'global_disaster', '#ef4444', '#ff8c00'],
            ...
            return (
              <Popup longitude={lng} latitude={lat} closeButton={false} closeOnClick={false} onClose={() => onEntityClick?.(null)} className="threat-popup" maxWidth="320px">
                <div className="map-popup min-w-[220px]" style={{ borderColor: `${accent}66`, background: 'var(--bg-secondary)' }}>
                  ...
                  {props.verification && (
                    <div className="map-popup-row">Durum: <span className={props.verification === 'approved' ? 'text-green-400 font-bold' : 'text-yellow-400'}>{props.verification.toUpperCase()}</span></div>
                  )}
                  {props.severity && (
              ...
              >
                <div className="map-popup border border-orange-500/30">
                  ...
                  {train.speed_kmh != null && (
                    <div className="map-popup-row">
                      Hız: <span className="text-[#44ff88]">{train.speed_kmh} km/h</span>
                    </div>
                  )}
                  <div className="map-popup-row">
                    Durum: <span className={train.status?.toLowerCase().includes('late') || train.status?.toLowerCase().includes('delay')
            ...
            return (
              ...
              >
                <div className="map-popup border border-red-500/40">
                  ...
                  {uav.uav_type && (
                    <div className="map-popup-row">
                      Sınıflandırma: <span className="text-[#ffcc00]">{uav.uav_type}</span>
                    </div>
                  )}
                  {uav.country && (
                    <div className="map-popup-row">
                      Ülke/Kayıt: <span className="text-white">{uav.country}</span>
                    </div>
                  )}
                  {uav.icao24 && (
                    <div className="map-popup-row">
                  ...
                  <div className="map-popup-row">
                    İrtifa: <span className="text-[#44ff88]">{uav.alt?.toLocaleString()} m</span>
                  </div>
                  {(uav.speed_knots ?? 0) > 0 && (
                    <div className="map-popup-row">
                      Hız: <span className="text-[#00e5ff]">{uav.speed_knots} kn</span>
                    </div>
                  )}
                  {uav.squawk && (
                    <div className="map-popup-row">
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <div className="map-popup-title text-purple-400">
                      {String(props.name || 'BİLİNMEYEN UYDU').toUpperCase()}
                    </div>
                    <button
                      onClick={() => onEntityClick?.(null)}
                      className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] ml-2"
                  ...
                  {Number(props.alt_km || 0) > 0 && (
                    <div className="map-popup-row mt-1">
                      İrtifa: <span className="text-purple-400">{Number(props.alt_km).toFixed(0)} km</span>
                    </div>
                  )}
                  ...
                  {String(props.status || '') !== '' && (
                    <div className="map-popup-row">
                      Durum: <span className="text-[#888]">{String(props.status)}</span>
                    </div>
                  )}
                  <div className="map-popup-row text-[10px] text-[#555] mt-1">
                    {lat.toFixed(4)}°, {lng.toFixed(4)}°
