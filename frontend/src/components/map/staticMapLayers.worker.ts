...
export type StaticMapLayersDataPayload = {
  ...
  gdelt?: GDELTIncident[];
  globalDisasters?: GlobalDisasterEvent[];
...
function buildStaticLayers(payload: StaticMapLayersBuildPayload): StaticMapLayersResult {
  ...
  return {
    ...
    militaryBasesGeoJSON: payload.activeLayers.military_bases
      ...
      : null,
    gdeltGeoJSON: payload.activeLayers.global_incidents
      ? (() => {
          const gdelt = buildGdeltGeoJSON(staticData.gdelt);
          const disasters = buildGlobalDisastersGeoJSON(staticData.globalDisasters, inView);
          if (!gdelt) return disasters;
          if (!disasters) return gdelt;
          return { type: 'FeatureCollection', features: [...gdelt.features, ...disasters.features] };
        })()
      : null,
