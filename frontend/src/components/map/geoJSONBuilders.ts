...
export function buildGlobalDisastersGeoJSON(events?: GlobalDisasterEvent[], inView?: InViewFilter): FC {
  if (!events?.length) return null;
  const features = events
    .filter((item) => Number.isFinite(Number(item.lat)) && Number.isFinite(Number(item.lng)))
    .filter((item) => !inView || inView(Number(item.lat), Number(item.lng)))
    .map((item) => ({
      type: 'Feature' as const,
      geometry: { type: 'Point' as const, coordinates: [Number(item.lng), Number(item.lat)] },
      properties: {
        id: String(item.id || `${item.provider}:${item.title}`),
        type: 'global_disaster',
        name: item.title || 'Doğal afet',
        provider: item.provider || 'Açık kaynak',
