...
function ConnectModalBody({ apiEndpoint, handleCopy, copied }: ConnectModalBodyProps) {
  ...
  const buildSnippet = (key: string) => {
    ...
    lines.push('# Every direct Gokdogan API request must be HMAC-SHA256 signed with:');
    lines.push('#   X-SB-Timestamp, X-SB-Nonce, X-SB-Signature');
    lines.push('# Signature input: METHOD|path|timestamp|nonce|sha256(body)');
    lines.push('# Use the GökdoğanClient/openclaw skill helper so requests are signed automatically.');
    lines.push('');
    lines.push('# OPERATING DIRECTIVE:');
    lines.push('# You are a remote OpenClaw agent connected to Gokdogan.');
    lines.push('# Gokdogan is a live intelligence and telemetry platform, not a narrow single-purpose API.');
    ...
    lines.push("# If something is absent, say 'not found in current telemetry' or 'not present in the current dataset' rather than saying Gokdogan cannot track it.");
    ...
    lines.push('# Gokdogan has a full SAR (Synthetic Aperture Radar) layer that detects ground changes through cloud cover, at night, anywhere on Earth.');
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-amber-200/80 leading-relaxed">
                Bir YZ ajanı bağlamak, ajanın Gokdogan verilerinize erişmesini sağlar.
            ...
            <div className="p-3 space-y-3 max-h-[60vh] overflow-y-auto styled-scrollbar">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-mono text-sky-400 tracking-widest">UYDU GÖRÜNTÜLERİ</span>
                </div>
