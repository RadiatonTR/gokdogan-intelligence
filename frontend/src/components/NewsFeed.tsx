...
import MovementHistoryBlock from '@/components/MovementHistoryBlock';
...
const AIRCRAFT_WIKI: Record<string, string> = {
    ...
    RC135: 'Boeing RC-135', RC35: 'Boeing RC-135', R185: 'Boeing RC-135',
...
function NewsFeedInner({ selectedEntity, regionDossier, regionDossierLoading, gtDossier, gtDossierLoading, onArticleClick, onExpandEntityGraph }: { selectedEntity?: SelectedEntity | null, regionDossier?: RegionDossier | null, regionDossierLoading?: boolean, gtDossier?: import('@/types/dashboard').GtDossier | null, gtDossierLoading?: boolean, onArticleClick?: (idx: number, lat?: number, lng?: number, title?: string) => void, onExpandEntityGraph?: () => void }) {
            ...
            >
                ...
                <div className="p-4 flex flex-col gap-2 text-[10px]">
                    ...
                    <div className="text-green-300/90">
                        Shodan verisi · Operatörün sağladığı API anahtarı · Yerel oturum katmanı
                    </div>
                    {shodanLoading && (
                ...
                >
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            <span className="text-[var(--text-muted)] text-[10px]">OPERATÖR</span>
                        ...
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            <span className="text-[var(--text-muted)] text-[10px]">TESCİL</span>
                        ...
                        {flight.squawk && (
                            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                                ...
                                <span className={`text-xs font-bold ${flight.squawk === '7700' ? 'text-red-400 animate-pulse' : flight.squawk === '7600' ? 'text-yellow-400' : 'text-[var(--text-primary)]'}`}>{flight.squawk}{flight.squawk === '7700' ? ' ⚠ ACİL DURUM' : flight.squawk === '7600' ? ' İLETİŞİM KAYBI' : ''}</span>
                            </div>
                        )}
                        ...
                        {flight.icao24 && (
                            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                                <span className="text-[var(--text-muted)] text-[10px]">UÇUŞ KAYDI</span>
                ...
                >
                    <div className="p-3 border-b border-[var(--border-primary)]/30 bg-[var(--bg-secondary)]/40 flex justify-between items-center">
                        <h2 className={`text-xs tracking-widest font-bold ${selectedEntity.type === 'military_flight' ? 'text-red-400' : selectedEntity.type === 'private_flight' ? 'text-orange-400' : selectedEntity.type === 'private_jet' ? 'text-purple-400' : 'text-cyan-400'} flex items-center gap-2`}>
                            {selectedEntity.type === 'military_flight' ? "ASKERÎ HAVA ARACI" : selectedEntity.type === 'private_flight' ? "ÖZEL / GENEL HAVACILIK" : selectedEntity.type === 'private_jet' ? "ÖZEL JET" : "TİCARİ UÇUŞ TRANSPONDERİ"}
                        </h2>
                        <span className="text-[10px] text-[var(--text-muted)] font-mono">TRK: {callsign}</span>
                    </div>
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        ...
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            <span className="text-[var(--text-muted)] text-[10px]">HAVA ARACI MODELİ</span>
                        ...
                        />
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            <span className="text-[var(--text-muted)] text-[10px]">ROTA</span>
    ...
    if (selectedEntity?.type === 'global_disaster') {
        const props = (selectedEntity.extra || {}) as Record<string, any>;
        return (
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full bg-black/85 border border-red-800 z-10 font-mono pointer-events-auto overflow-hidden flex-shrink-0">
                <div className="p-3 border-b border-red-500/30 bg-red-950/35 flex justify-between items-center">
                    <h2 className="text-xs tracking-widest font-bold text-red-300 flex items-center gap-2"><AlertTriangle size={14} /> KÜRESEL AFET OLAYI</h2>
                    <span className="text-[9px] text-red-600">{String(props.provider || 'Açık kaynak')}</span>
                </div>
                <div className="p-4 flex flex-col gap-2 text-[10px]">
                    <div className="text-white font-bold">{String(props.name || selectedEntity.name || 'Doğal afet')}</div>
                    <div className="flex justify-between border-b border-red-950/60 pb-1"><span className="text-red-700">KATEGORİ</span><span>{String(props.categories || 'Doğal olay')}</span></div>
                    <div className="flex justify-between border-b border-red-950/60 pb-1"><span className="text-red-700">UYARI DÜZEYİ</span><span>{String(props.alert_level || '—')}</span></div>
                    <div className="flex justify-between border-b border-red-950/60 pb-1"><span className="text-red-700">OLAY ZAMANI</span><span>{String(props.event_time || '—')}</span></div>
                    {props.url ? <a href={String(props.url)} target="_blank" rel="noreferrer" className="text-cyan-400 underline">RESMÎ / KAYNAK SAYFASINI AÇ ↗</a> : null}
                </div>
            </motion.div>
        );
    }
                ...
                >
                    <div className="p-3 border-b border-orange-500/30 bg-orange-950/40 flex justify-between items-center">
                        <h2 className="text-xs tracking-widest font-bold text-orange-400 flex items-center gap-2">
                            <AlertTriangle size={14} className="text-orange-400" /> ASKERÎ / JEOPOLİTİK OLAY KÜMESİ
                        </h2>
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            ...
                            <span className="text-[var(--text-primary)] text-xs font-bold text-right ml-4">{props.name || 'BİLİNMEYEN BÖLGE'}</span>
                        </div>
                        <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                            <span className="text-[var(--text-muted)] text-[10px]">HABER SAYISI</span>
                            <span className="text-orange-400 text-xs font-bold">{props.count || 1}</span>
                        </div>
                        <div className="flex flex-col gap-2 mt-2">
                            <span className="text-[var(--text-muted)] text-[10px]">SON RAPORLAR:</span>
                            <div className="flex flex-col gap-1 max-h-[250px] overflow-y-auto styled-scrollbar">
                                {(() => {
                                    ...
                                    if (urls.length === 0) return <span className="text-[var(--text-muted)] text-[10px]">Kaynak makale bulunamadı.</span>;
