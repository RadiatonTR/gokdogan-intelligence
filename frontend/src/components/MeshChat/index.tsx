...
const MeshChat = React.memo(function MeshChat(props: MeshChatProps) {
  ...
  const {
    ...
    localMeshtasticDevice,
    localMeshtasticBusy,
    refreshLocalMeshtasticDevice,
    connectLocalMeshtasticDevice,
    disconnectLocalMeshtasticDevice,
    // Identity
    identity,
    publicIdentity,
                  ...
                  <div className="flex-1 overflow-y-auto styled-scrollbar px-3 py-1.5 border-l-2 border-cyan-800/25">
                    ...
                    {meshView === 'settings' && (
                      <div className="space-y-2 py-1 text-[11px] font-mono">
                        <div className="border border-cyan-800/35 bg-cyan-950/10 p-2">
                          <div className="flex items-center justify-between gap-2">
                            <div>
                              ...
                              <div className="mt-1 text-[10px] text-[var(--text-muted)] leading-[1.5]">
                                Meshtastic MQTT, Wormhole kanalından ayrıdır. MQTT etkinleştirildiğinde Meshtastic Sohbet için özel Wormhole hattı devre dışı bırakılır.
                              </div>
                            </div>
                            <span
                          ...
                          {meshMqttSettings?.runtime?.last_error && (
                            <div className="mt-2 text-red-300/80">
                              SON HATA: {meshMqttSettings.runtime.last_error}
                            </div>
                          )}
                          {meshMqttRunning && !meshMqttConnected && !meshMqttSettings?.runtime?.last_error && (
                            <div className="mt-2 text-amber-300/80">
                              MQTT köprüsü başlatılıyor. Sunucu bağlantısı kurulduğunda canlı mesajlar görünür.
                            </div>
                          )}
                        </div>

                        <div className="border border-emerald-800/35 bg-emerald-950/10 p-2">
                          <div className="flex items-center justify-between gap-2">
                            <div>
                              <div className="text-emerald-300 tracking-[0.18em]">YEREL MESHTASTIC CİHAZI</div>
                              <div className="mt-1 text-[10px] text-[var(--text-muted)] leading-[1.5]">
                                USB/seri bağlı gerçek Meshtastic radyoyu otomatik algılar. Cihaz yoksa genel MQTT hattı kullanılmaya devam eder.
                              </div>
                            </div>
                            <span className={`shrink-0 border px-2 py-1 text-[10px] ${localMeshtasticDevice?.connected ? 'border-green-500/40 text-green-300' : 'border-slate-700 text-slate-400'}`}>
                              {localMeshtasticDevice?.connected ? 'CİHAZ BAĞLI' : localMeshtasticDevice?.sdk_available === false ? 'SDK YOK' : 'CİHAZ YOK'}
                            </span>
                          </div>
                          <div className="mt-2 flex gap-2">
                            <button onClick={() => void connectLocalMeshtasticDevice()} disabled={localMeshtasticBusy || localMeshtasticDevice?.connected} className="border border-green-600/40 px-2 py-1 text-green-300 disabled:opacity-40">OTOMATİK BAĞLAN</button>
                            <button onClick={() => void disconnectLocalMeshtasticDevice()} disabled={localMeshtasticBusy || !localMeshtasticDevice?.connected} className="border border-red-600/35 px-2 py-1 text-red-300 disabled:opacity-40">BAĞLANTIYI KES</button>
                            <button onClick={() => void refreshLocalMeshtasticDevice()} disabled={localMeshtasticBusy} className="border border-cyan-700/40 px-2 py-1 text-cyan-300 disabled:opacity-40">DURUMU YENİLE</button>
                          </div>
                          {localMeshtasticDevice?.connected && <div className="mt-2 text-[10px] text-emerald-200/80">Bağlantı: {localMeshtasticDevice.mode || 'serial'} • Hedef: {localMeshtasticDevice.target || 'otomatik'} • Düğümler: {localMeshtasticDevice.node_count || 0}</div>}
                          {localMeshtasticDevice?.last_error && <div className="mt-2 text-[10px] text-red-300/80">{localMeshtasticDevice.last_error}</div>}
                        </div>
                        ...
                        <label className="block space-y-1">
                          <span className="text-[var(--text-muted)]">SUNUCU KULLANICISI (isteğe bağlı)</span>
                          <input
                            value={meshMqttForm.username}
                            onChange={(e) => setMeshMqttForm((prev) => ({ ...prev, username: e.target.value }))}
                            placeholder="boş bırakılırsa genel Meshtastic varsayılanı kullanılır"
                            className="w-full border border-[var(--border-primary)] bg-black/30 px-2 py-1 text-cyan-200 outline-none focus:border-cyan-500/50"
                          />
                        </label>
                        ...
                        <label className="block space-y-1">
                          <span className="text-[var(--text-muted)]">
                            SUNUCU PAROLASI {meshMqttSettings?.uses_default_credentials ? '(genel varsayılan)' : meshMqttSettings?.has_password ? '(kaydedildi)' : ''}
                          </span>
                          <input
                            type="password"
                            ...
                            placeholder={
                              meshMqttSettings?.uses_default_credentials
                                ? 'boş bırakılırsa genel Meshtastic varsayılanı kullanılır'
                                : meshMqttSettings?.has_password
                                  ? 'kaydedilmiş parolayı korumak için boş bırakın'
                                  : 'boş bırakılırsa genel Meshtastic varsayılanı kullanılır'
                            }
                        ...
                        <label className="block space-y-1">
                          <span className="text-[var(--text-muted)]">
                            KANAL PSK HEX {meshMqttSettings?.has_psk ? '(kaydedildi)' : '(boşsa varsayılan LongFast)'}
                          </span>
                          <input
                            type="password"
                            value={meshMqttForm.psk}
                            onChange={(e) => setMeshMqttForm((prev) => ({ ...prev, psk: e.target.value }))}
                            placeholder="boş bırakılırsa varsayılan LongFast anahtarı kullanılır"
                            className="w-full border border-[var(--border-primary)] bg-black/30 px-2 py-1 text-cyan-200 outline-none placeholder:text-[var(--text-muted)] focus:border-cyan-500/50"
                          />
                        </label>
                        ...
                        <label className="flex items-center gap-2 border border-[var(--border-primary)]/40 bg-black/20 px-2 py-1 text-cyan-200">
                          ...
                          />
                          VARSAYILAN GENEL KÖKLER
                        </label>

                        <label className="block space-y-1">
                          <span className="text-[var(--text-muted)]">EK KÖKLER</span>
                          <input
                            value={meshMqttForm.extra_roots}
                            onChange={(e) => setMeshMqttForm((prev) => ({ ...prev, extra_roots: e.target.value }))}
                            placeholder="virgülle ayrılmış, isteğe bağlı"
                            className="w-full border border-[var(--border-primary)] bg-black/30 px-2 py-1 text-cyan-200 outline-none placeholder:text-[var(--text-muted)] focus:border-cyan-500/50"
                          />
                        </label>
                    ...
                    {!canUsePublicMeshInput && meshView !== 'settings' && (
                      <div className="text-[12px] font-mono text-green-300/70 text-center py-4 leading-[1.65]">
                        Meshtastic Sohbet kapalı. Genel Meshtastic MQTT hattına bağlanmak için etkinleştirin.
                      </div>
                    )}
                    ...
                    {canUsePublicMeshInput && meshView === 'inbox' && (
                      <>
                        {!activePublicMeshAddress && (
                          <div className="text-[12px] font-mono text-[var(--text-muted)] text-center py-4 leading-[1.65]">
                            Doğrudan Meshtastic trafiğini görmek için genel bir mesh kimliği oluşturun veya yükleyin.
                          </div>
                        )}
