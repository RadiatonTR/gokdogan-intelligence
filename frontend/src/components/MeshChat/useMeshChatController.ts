...
interface LocalMeshtasticDeviceStatus {
  ok?: boolean;
...
export function useMeshChatController({
  ...
  const [localMeshtasticDevice, setLocalMeshtasticDevice] = useState<LocalMeshtasticDeviceStatus | null>(null);
  const [localMeshtasticBusy, setLocalMeshtasticBusy] = useState(false);
  ...
  const refreshLocalMeshtasticDevice = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/sigint/meshtastic/device`, { cache: 'no-store' });
      if (!res.ok) return null;
      const data = (await res.json()) as LocalMeshtasticDeviceStatus;
      setLocalMeshtasticDevice(data);
      return data;
    } catch {
      return null;
    }
  ...
  const connectLocalMeshtasticDevice = useCallback(async () => {
    setLocalMeshtasticBusy(true);
    try {
      const res = await fetch(`${API_BASE}/api/sigint/meshtastic/device/connect`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'serial', target: '' }),
      });
      const data = (await res.json().catch(() => ({}))) as LocalMeshtasticDeviceStatus;
      setLocalMeshtasticDevice(data);
      return data;
    } finally {
      setLocalMeshtasticBusy(false);
    }
  }, []);

  const disconnectLocalMeshtasticDevice = useCallback(async () => {
    setLocalMeshtasticBusy(true);
    try {
      const res = await fetch(`${API_BASE}/api/sigint/meshtastic/device/disconnect`, { method: 'POST' });
      const data = (await res.json().catch(() => ({}))) as LocalMeshtasticDeviceStatus;
      setLocalMeshtasticDevice(data);
      return data;
    } finally {
  ...
  useEffect(() => {
    ...
    let alive = true;
    const tick = async () => {
      const [data] = await Promise.all([refreshMeshMqttSettings(), refreshLocalMeshtasticDevice()]);
