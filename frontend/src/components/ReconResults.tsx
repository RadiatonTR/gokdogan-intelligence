...
function ThreatsDossier({ data }: { data: Record<string, unknown> }) {
  ...
  const abuse = asRecord(data.abuseipdb) || {};
  ...
  return (
    <DossierShell title="THREAT INTEL DOSSIER">
      ...
      {'abuse_confidence_score' in abuse ? (
        <DossierRow label="ABUSEIPDB SCORE" value={`${fmt(abuse.abuse_confidence_score)} / 100`} highlight={Number(abuse.abuse_confidence_score || 0) >= 75 ? 'red' : Number(abuse.abuse_confidence_score || 0) >= 25 ? 'amber' : 'green'} />
      ) : null}
      {'total_reports' in abuse ? <DossierRow label="ABUSEIPDB REPORTS" value={fmt(abuse.total_reports)} /> : null}
