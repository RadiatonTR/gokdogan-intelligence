  ...
  return (
    <div className="bg-red-950/20 border border-red-500/40 p-3 -mt-1 shadow-[0_0_15px_rgba(220,38,38,0.1)]">
      ...
      <div className="grid grid-cols-[1fr_1fr_auto] items-center gap-2 mb-2">
        ...
        >
          <FastForward size={10} />
          {isScanning ? 'TARANIYOR...' : 'OTOMATİK TARA'}
        </button>
        <input
          type="range"
          min="0"
  ...
  const sections = [
    {
      ...
      layers: [
        ...
        },
        {
          id: 'uav',
          name: t('layers.uav'),
          source: 'ADS-B / kamu yayını',
          count: data?.uavs?.length || 0,
          icon: Radar,
        },
        {
          id: 'tracked',
          ...
          source:
            (data?.sar_anomalies?.length
              ...
              : (data?.sar_scenes?.length
                ? `Yalnız katalog · ${data.sar_scenes.length} Sentinel-1 geçişi · Uyarılar: ücretsiz hesapla etkinleştir →`
                : 'Yalnız katalog (ücretsiz) · Uyarılar: ücretsiz hesapla etkinleştir →')),
    ...
    },
    {
      ...
      layers: [
        ...
        },
        {
          ...
          source: 'GDELT',
          count: (data?.gdelt?.length || 0) + (data?.global_disasters?.events?.length || 0),
          icon: Activity,
        },
        {
          id: 'telegram_osint',
        ...
        <div className="flex items-center gap-1.5">
          ...
          {onLegendClick && (
            <button
              ...
              title="Harita lejandı / simge anahtarı"
            >
              <BookOpen size={12} />
