...
export default function AIQueryView({ onBack }: AIQueryViewProps) {
  const [messages, setMessages] = useState<AIMessage[]>([
    {
      role: 'system',
      content: `🌍📡 GÖKDOĞAN YZ YARDIMCISI ÇEVRİMİÇİ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Gokdogan OSINT platformuna bağlı.
I can query telemetry, place pins on the map,
search satellite imagery, aggregate news,
and access all 30+ data layers.

Başlamak için bir soru veya komut yazın.
Use "help" to see capabilities.`,
  ...
  const processQuery = useCallback(async (query: string) => {
    ...
    if (lowerQuery === 'help') {
      return {
        content: `🌍🔍 KULLANILABİLİR KOMUTLAR:
━━━━━━━━━━━━━━━━━━━━━━━━━━━
TELEMETRY QUERIES:
  ...
  • "help"    — This message`,
    ...
    try {
      ...
      if (lowerQuery === 'status') {
        ...
        return {
          content: `🌍✅ GÖKDOĞAN YZ DURUMU:
━━━━━━━━━━━━━━━━━━━━━━━━━━━
Durum: ${data.status || 'ÇEVRİMİÇİ'}
      ...
      if (lowerQuery.startsWith('pin ')) {
        ...
        if (parts.length >= 3) {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            ...
            const data = await resp.json();
            return {
              content: `🌍📌 GÖKDOĞAN HARİTA İŞARETİ:
...
${pinList || '  No pins placed yet.'}`,
      ...
      if (lowerQuery === 'clear pins') {
        ...
        return { content: '🌍❌ GÖKDOĞAN TEMİZLİĞİ:\nTüm YZ istihbarat işaretleri temizlendi.' };
      }
      ...
      if (lowerQuery === 'snapshot' || lowerQuery === 'take snapshot') {
        ...
        const data = await resp.json();
        return {
          content: `🌍🕰️ GÖKDOĞAN ZAMAN MAKİNESİ:
Snapshot taken!
  🆔 ${data.snapshot_id}
  🕐 ${data.timestamp}
...
Tam komut listesi için “help” yazın.`,
      };
    } catch (error) {
      return {
        content: `🌍⚠️ GÖKDOĞAN UYARISI:
Sorgu başarısız: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}
Gokdogan arka ucunun localhost:8000 üzerinde çalıştığından emin olun.`,
      };
    }
  }, []);
      ...
      {showConnect && (
        <div className="border-b border-purple-900/40 bg-purple-950/15 px-4 py-4 shrink-0 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-3 text-sm font-mono">
            {/* API Endpoint */}
            <div>
              <div className="text-[11px] text-gray-500 uppercase tracking-widest mb-1">Gokdogan API Uç Noktanız</div>
              <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="bg-black/60 border border-gray-800/40 rounded-sm p-3 space-y-2 text-[13px] leading-relaxed">
                <p className="text-cyan-400 font-bold">Adım 1: Gokdogan Skill’i Kurun</p>
                <p className="text-gray-400">Kopyalayın: <code className="text-purple-300 bg-purple-900/30 px-1">openclaw-skills/shadowbroker/</code> folder into your OpenClaw&apos;s skills directory.</p>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center bg-gray-900/40 border border-gray-700/40 focus-within:border-purple-700/60 transition-colors rounded-sm">
            ...
            <input
              ...
              placeholder="Bir şey sorun... (komutlar için 'help' yazın)"
