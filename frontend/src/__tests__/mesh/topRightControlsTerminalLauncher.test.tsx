...
describe('TopRightControls terminal launcher', () => {
  ...
  it('opens the terminal immediately while Wormhole prep continues in the background', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'TERMİNAL' }));
    expect(await screen.findByRole('button', { name: 'WORMHOLE’U ETKİNLEŞTİR' })).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: 'WORMHOLE’U ETKİNLEŞTİR' }));

    await waitFor(() => expect(onTerminalToggle).toHaveBeenCalledTimes(1));
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: 'WORMHOLE’U ETKİNLEŞTİR' })).toBeNull(),
    );
