...
describe('buildGlobalDisastersGeoJSON', () => {
  it('maps only geolocated public disaster events', () => {
    const events: GlobalDisasterEvent[] = [
      { id: 'eonet:1', provider: 'NASA EONET', title: 'Wildfire', lat: 41, lng: 29, categories: ['Wildfires'], alert_level: 'ORANGE', url: 'https://example.gov' },
      { id: 'gdacs:2', provider: 'GDACS', title: 'No coords' },
    ];
    const result = buildGlobalDisastersGeoJSON(events);
    expect(result!.features).toHaveLength(1);
    expect(result!.features[0].properties?.type).toBe('global_disaster');
    expect(result!.features[0].properties?.provider).toBe('NASA EONET');
    expect(result!.features[0].geometry).toEqual({ type: 'Point', coordinates: [29, 41] });
  });
});
