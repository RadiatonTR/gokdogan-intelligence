...
describe('MaplibreViewer decomposition — CCTV proxy URL behavior', () => {
  ...
  it('CCTV entries without a public media URL are classified as metadata-only', () => {
    expect(viewer).toContain("!rawUrl ? 'none'");
    const modal = readComp('MaplibreViewer/CctvFullscreenModal.tsx');
    expect(modal).toContain('KAMUYA AÇIK YAYIN YOK');
    expect(modal).toContain('Özel/kapalı kamera akışlarına erişim denenmez.');
  });
