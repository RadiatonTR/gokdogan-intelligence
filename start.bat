@echo off
title Gokdogan - Kuresel Istihbarat Merkezi

echo ===================================================
echo     G O K D O G A N   --   BASLATMA
echo ===================================================
echo.
...
if not defined PYTHON_EXE (
    echo [!] HATA: Python kurulu degil veya PATH icinde degil.
...
where npm >nul 2>&1
if %errorlevel% neq 0 (
    echo [!] HATA: Node.js/npm kurulu degil veya PATH icinde degil.
...
for /f "tokens=1 delims= " %%v in ('node --version 2^>^&1') do echo [*] Node.js bulundu: %%v
...
start "Gokdogan Runtime" powershell.exe -NoProfile -ExecutionPolicy Bypass -NoExit -File "%ROOT%\scripts\run-windows-runtime.ps1" -Root "%ROOT%"
exit /b 0

echo.
echo ===================================================
echo   Gokdogan durdu. Yukaridaki hatalari kontrol edin.
echo ===================================================
pause
